...
makeCHAIN <- function(intro, arena, threshold = .2, limits = c(1,1.5)){
...
ggplot(alldf, aes(x = Var1/10, y = value, col = stdev)) + geom_path(lwd = 1) + geom_point(size = 4) + scale_x_continuous(breaks = seq(.1, 1, .1)) + scale_color_discrete(name = "Size \nDistribution") +  facet_grid(size~Var2, scales = "free_y") + theme_bw() + xlab("Quantile") + ylab("Chain Length") 
